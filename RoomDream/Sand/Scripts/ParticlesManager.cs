...
public class ParticlesManager : MonoBehaviour {
  ...
  /*  private void OnDrawGizmos()
	...
	void Update () {
        ...
        computeShader.SetInt("shouldMove", checkHand? 1 : 0);//Input.GetMouseButton(0) ? 1 : 0);
        var bodyPosition = GetBodyPosition(); // GetMousePosition();
        computeShader.SetFloats("mousePosition", bodyPosition);
    ...
    float[] GetBodyPosition()
    {
        GameObject handLeft = GameObject.Find("HandLeft");
        if (handLeft)
        {
            checkHand = true;
           // Debug.Log(" hand HandLeft : " + handLeft.transform.position.x + " , " + handLeft.transform.position.y);
            return new float[] { handLeft.transform.position.x, handLeft.transform.position.y };
        }
        else
        {
