...
public class MeasureDepth : MonoBehaviour
{
    ...
    // Cutoffs
    [Range(0, 1.0f)]
    public float mDepthSensitivity = 1; // basicly detect depth.
    ...
    private readonly Vector2Int mDepthResolution = new Vector2Int(512, 424); // 해상도
    ...
    private void Awake()
    {
        ...
        mCamera = GameObject.Find("OfflinePivot").transform.Find("BubbleCamera").GetComponent<Camera>(); // Camera.main;
...
/*    private void OnGUI()
    ...
    private List<ValidPoint> DepthToColor()
    {
        ...
        //Map - Converting depth data to camerPoints or colorPoints
        ...
        //Filter - filtering data and put them into the validpoints.
        for(int i = 0; i< mDepthResolution.x / 8; i++)
        {
            for(int j = 0; j< mDepthResolution.y / 8; j++)
            {
                //Sample index - make 2d array to 1d array
    ...
    private List<Vector2> FilterToTrigger(List<ValidPoint> validPoints) // filtering valipdPoints to list up triggerPoints.
    {
    ...
    private Texture2D CreateTexture(List<ValidPoint> validPoints)
    {
        ...
        newTexture.Apply(); // change된 것을 반영해야하니까.
    ...
    private Rect CreateRect(List<ValidPoint> points)
    {
        ...
        Vector2 screeenBottomRight = ScreenToCamera(bottomRight);


        // Rect dimensions 
        int width = (int)(screeenBottomRight.x - screenTopLeft.x);
        int height = (int)(screeenBottomRight.y = screenTopLeft.y);

        //Create
        Vector2 size = new Vector2(width, height);
        Rect rect = new Rect(screenTopLeft, size); // Rect(position, size) 로 한 것.
    ...
    {
        Vector2 normailizedScreen = new Vector2(Mathf.InverseLerp(0, 1920, screenPosition.x), Mathf.InverseLerp(0, 1080, screenPosition.y));

        Vector2 screenPoint = new Vector2(normailizedScreen.x * mCamera.pixelWidth, normailizedScreen.y * mCamera.pixelHeight);
...
public class ValidPoint 
{
    ...
    public bool withInWallDepth = false; // 나중에 쓸 것. wall 체킹한 것인지 아닌지 확인용으로 사용.
